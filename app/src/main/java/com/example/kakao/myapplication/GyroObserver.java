...
public class GyroObserver implements SensorEventListener {
    ...
    // The PanoImageViews to be notified when the device rotate.
    private LinkedList<PanoImageView> mViews = new LinkedList<>();
    ...
    void addPanoImageView(PanoImageView view) {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (rotateY > rotateX + rotateZ) {
            ...
            for (PanoImageView view: mViews) {
                if (view != null) {
                    view.updateProgress((float) (mRotateRadianY / mMaxRotateRadian) - progress);
                    progress = (float) (mRotateRadianY / mMaxRotateRadian);
                }
            }
        }
        mLastTimestamp = event.timestamp;
    ...
    public void setMaxRotateRadian(double maxRotateRadian) {
        if (maxRotateRadian <= 0 || maxRotateRadian > Math.PI) {
            throw new IllegalArgumentException("The maxRotateRadian must be between (0, π].");
        }
